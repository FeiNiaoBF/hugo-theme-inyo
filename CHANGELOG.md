...
- 归档页改为编辑型年谱：按年份显示放大的 `二〇二六`，末尾使用朱红圆印章「年」；所有语言统一采用这套年份视觉。
- 归档文章整行可点击，标题置左、日期 folio 置右，并为标签结果页增加返回标签索引入口。

### Changed

- 标签索引的文章数改为两位等宽 folio，并提供至少 24px 的点击目标。
- 归档与标签页同步更新亮暗色样式、设计规范和验证门禁。
